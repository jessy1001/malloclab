...
static void* find_fit(size_t asize)
{
	...
	for(j = i; j <= 32; j++){
		...
		if(GET(seg) != NULL){ //different
			best = (1<<(j + 1));
			for(bp = PRED_FREEP(seg); bp != NULL; bp = PRED_FREEP(bp)){
				if (asize <= GET_SIZE(HDRP(bp)) && GET_SIZE(HDRP(bp)) - asize < best){ //전에 배정한 best보다 더 딱 맞는다
					best_bp = bp;
...
void* mm_realloc(void* bp, size_t size) 								  // reallocation--------------------------------------------------------------------
{
	...
	if (!GET_ALLOC(HDRP(NEXT_BLKP(old_dp))) && old_size + old_next_size >= size) {  // ���� �ڿ� free ������ �ְ�, ���ݰ� ������ �� �ʿ��� size�� ���� �� �ִٸ�
		size_t asize;
		...
		removeBlock(NEXT_BLKP(old_dp));												// �� free ������ ��ũ�� ������

		if ((old_size + old_next_size - asize) >= (2 * DSIZE)) {					// �����ϰ� �ڸ��� ������ SPLIT ���ش�.
			PUT(HDRP(bp), PACK(asize, 1));
			PUT(FTRP(bp), PACK(asize, 1));
